...
2. 절전 모드 (휴면 모드)
  ...
  절전 모드는 24시간 가동되는 스마트 화분의 전력소비량을
  낮추기 위해, 광에 의한 식물에게 스트레스를 주는 행위를 방지하기 위한 모드입니다
  
  절전 모드 상태에서는 광 센서를 통해 감지된 빛의 양에 관계없이,
  LED와 LCD 디스플레이 백라이트가 비활성화된 상태로 지속됩니다
  
  
스마트 LED 화분은 전원 연결점을 기준으로
3시간 간격으로 자동으로 1번 모드(LED 모드)와 2번 모드(절전 모드)를
번갈아가며 모드 교체를 하게 됩니다
```
</br>
